...
do
	...
	echo "Running CPU version with n=m=$size"
	./prog -n $size -m $size -t -g > results/cpu-size-$size.txt
	echo "Done"
done
...
do
	...
	do
		size=${MAT_SIZES[m_index]}
		echo "Running CUDA version with n=m=$size and block size = $block_size"
		./prog -n $size -m $size -t -c -b $block_size > results/cuda-size-$size-block-size-$block_size.txt
		echo "Done"
	done
done
